...
unique_ptr<BstNode<int>> BST_Node_Construct(int l, int r, const vector<int>& A){
  if(r<l) return nullptr;
  if(l == r) return std::make_unique<BstNode<int>>(A[l]);
  int m = l + ((r-l)/2);
  unique_ptr<BstNode<int>> curr = std::make_unique<BstNode<int>>(A[m]);
  curr->left = BST_Node_Construct(l, m-1, A);
  curr->right = BST_Node_Construct(m+1, r, A);
  return curr;
}
...
unique_ptr<BstNode<int>> BuildMinHeightBSTFromSortedArray(
  ...
  return BST_Node_Construct(0, A.size()-1, A);
}
