...
// checks if a root contains a key in its subtrees 
bool contains_node(const BstNode<int>* root, const int& val){
  const BstNode<int>* curr = root;
  if(!root || root->data == val) return false;
  while(curr)
  {
    if(curr->data == val) return true;
    if(curr->data > val) curr = curr->left.get();
...
bool PairIncludesAncestorAndDescendantOfM(
  ...
  return (contains_node(possible_anc_or_desc_0.get(), middle->data) && contains_node(middle.get(), possible_anc_or_desc_1->data)) ||
    (contains_node(possible_anc_or_desc_1.get(), middle->data) && contains_node(middle.get(), possible_anc_or_desc_0->data));
}
