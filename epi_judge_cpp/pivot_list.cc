...
shared_ptr<ListNode<int>> ListPivoting(const shared_ptr<ListNode<int>>& l,
  ...
  shared_ptr<ListNode<int>> lt_head(new ListNode<int>(0)), eq_head(new ListNode<int>(0)), gt_head(new ListNode<int>(0));
  shared_ptr<ListNode<int>> lt_it = lt_head, eq_it = eq_head, gt_it = gt_head, it = l;

  if(!it||!it->next) return l;
  ...
  gt_it->next = nullptr;
  eq_it->next = gt_head->next;
  lt_it->next = eq_head->next;
  return lt_head->next;
}
