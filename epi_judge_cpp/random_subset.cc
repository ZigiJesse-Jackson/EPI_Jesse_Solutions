...
vector<int> RandomSubset(int n, int k) {
  std::unordered_map<int, int> swapped_nums;
  // TODO - you fill in here.
  vector<int> res;
  for(int i=0;i<k;i++){
    int rand_num = (std::rand()%(k));
    auto find_i = swapped_nums.find(i);
    auto find_rand_num = swapped_nums.find(rand_num);
    auto end = swapped_nums.end();
    if(find_i==end&& find_rand_num==end){
      swapped_nums[i] = rand_num;
      swapped_nums[rand_num] = i;
    }
    else if(find_i==end){
      swapped_nums[i] = swapped_nums[rand_num];
      swapped_nums[rand_num] = i;
    }
    else if(find_rand_num==end){
      swapped_nums[rand_num] = swapped_nums[i];
      swapped_nums[i] = rand_num;
    }
    else{
      int temp = swapped_nums[i];
      swapped_nums[i] = swapped_nums[rand_num];
      swapped_nums[rand_num] = temp;
    }
  }

  for(int i=0;i<k;i++){
    res.push_back(swapped_nums[i]);
  }
