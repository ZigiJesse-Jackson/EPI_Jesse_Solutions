...
void CreateListOfLeavesHelper(
    const unique_ptr<BinaryTreeNode<int>> &node, vector<const unique_ptr<BinaryTreeNode<int>> *> &list)
{
  if(!node) return;
  if(!node->left && !node->right) {
    const unique_ptr<BinaryTreeNode<int>> *ptr = &node;
    list.push_back(ptr);
    return;
  }
  CreateListOfLeavesHelper(node->left, list);
  CreateListOfLeavesHelper(node->right, list);
}
    ...
    const unique_ptr<BinaryTreeNode<int>> &tree)
{
  ...
  CreateListOfLeavesHelper(tree, res);
    ...
    TimedExecutor &executor, const unique_ptr<BinaryTreeNode<int>> &tree)
{
  auto result = executor.Run([&]
                             { return CreateListOfLeaves(tree); });
